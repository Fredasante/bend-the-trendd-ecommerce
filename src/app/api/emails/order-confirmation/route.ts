...
const SHOP_EMAIL = "bend.the.trendd01@gmail.com";
...
export async function POST(req: Request) {
  try {
    ...
    const itemsText = items
      .map(
        (item: any) =>
          `${item.productSnapshot.name} - Qty: ${item.quantity} × GH₵${item.priceAtPurchase.toFixed(2)} = GH₵${(item.priceAtPurchase * item.quantity).toFixed(2)}`
      )
      .join("\n");
            ...
            <tr>
              <td align="center" style="padding: 40px 0; background-color: #f9f9f9;">
                <table role="presentation" style="width: 600px; max-width: 100%; border-collapse: collapse; background-color: #ffffff; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                  ...
                  <!-- Header -->
                  <tr>
                    <td style="padding: 40px 40px 20px; text-align: center; background-color: #382423; border-radius: 12px 12px 0 0;">
                  ...
                  <tr>
                    <td style="padding: 40px;">
                      ...
                      <div style="background-color: #f8f8f8; padding: 20px; border-radius: 8px; margin-bottom: 25px;">
                        ...
                        <p style="margin: 0; color: #382423; font-size: 18px; font-weight: bold; font-family: monospace;">${orderId}</p>
                      ...
                      <table role="presentation" style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                        <thead>
                          <tr>
                            <th style="padding: 12px; text-align: left; border-bottom: 2px solid #382423; color: #666666; font-size: 14px; font-weight: 600;">Item</th>
                            <th style="padding: 12px; text-align: right; border-bottom: 2px solid #382423; color: #666666; font-size: 14px; font-weight: 600;">Price</th>
                            <th style="padding: 12px; text-align: right; border-bottom: 2px solid #382423; color: #666666; font-size: 14px; font-weight: 600;">Total</th>
                          </tr>
                        </thead>
                        <tbody>
                          ${itemsHtml}
                  ...
                  <tr>
                    <td style="padding: 30px 40px; background-color: #f8f8f8; text-align: center; border-top: 1px solid #eeeeee; border-radius: 0 0 12px 12px;">
                      <p style="margin: 0; color: #666666; font-size: 14px;">
                        Bend The Trendd - Order Notification
                      </p>
                    </td>
                  </tr>
                </table>
    ...
    const { data, error } = await resend.emails.send({
      from: "Bend The Trendd <orders@bendthetrendd.com>",
      to: [SHOP_EMAIL],
